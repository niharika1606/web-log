...
class UserRecommendationSystem:
    def __init__(self, frequent_itemsets, total_transactions):
        ...
        print(f"DEBUG: Total transactions: {self.total_transactions}")
        print(f"DEBUG: Found {len(self.pattern_database)} pages with patterns")
    
    def _build_enhanced_pattern_database(self):
        """Build enhanced pattern database with proper support calculation"""
        pattern_db = defaultdict(list)
        
        # First, calculate actual support percentages correctly
        for itemset, support in self.frequent_itemsets:
            # CORRECT support calculation - divide by total transactions
            support_pct = (support / self.total_transactions) * 100
            
            # Only process meaningful patterns with valid support
            if len(itemset) >= 1 and support_pct <= 100:  # Sanity check
                ...
                if len(itemset) > 1:
                    for i in range(len(itemset)):
                        ...
                        # What comes AFTER this page in the sequence
                        if i < len(itemset) - 1:
                            next_page = itemset[i + 1]
                            pattern_db[current_page].append(('next', next_page, support_pct))
                        
                        # What comes BEFORE this page in the sequence  
                        if i > 0:
        ...
        for page in pattern_db:
            # Remove duplicates and keep highest support
            unique_patterns = {}
            ...
            # Convert back to list and sort by support
    ...
    def _get_global_popular_pages(self):
        """Get globally popular pages with CORRECT support calculation"""
        page_support = defaultdict(float)
        
        for itemset, support in self.frequent_itemsets:
            support_pct = (support / self.total_transactions) * 100
            if support_pct <= 100:  # Only include valid percentages
                for page in itemset:
                    # Use maximum support this page appears with
                    if support_pct > page_support[page]:
                        page_support[page] = support_pct
    ...
    def get_next_page_recommendations(self, current_page, top_n=5):
        """Get ACTUAL next page recommendations based on patterns"""
        recommendations = []
        
        # Get direct next page patterns from the database
        if current_page in self.pattern_database:
            next_pages = [(page, sup) for typ, page, sup in self.pattern_database[current_page] 
                         if typ == 'next' and sup <= 100]  # Only valid percentages
            
            if next_pages:
                recommendations.extend(next_pages)
        
        # If no direct patterns, try to infer from sequence positions
        if not recommendations and current_page in self.pattern_database:
            # Look for pages that frequently appear with this page
            related_pages = [(page, sup*0.7) for typ, page, sup in self.pattern_database[current_page] 
                           if typ == 'related' and sup <= 100]
            
            if related_pages:
                recommendations.extend(related_pages[:top_n])
        
        # Final fallback: contextual recommendations
        if not recommendations:
            contextual_recs = self.get_contextual_recommendations(current_page)
            recommendations = [(page, sup*0.5) for page, sup in contextual_recs]  # Lower confidence
        
        # Remove duplicates and return top N
        unique_recs = {}
        for page, support in recommendations:
            if page != current_page and support <= 100:  # Valid pages and percentages
        ...
        if not results:
            # Last resort: global popular pages (excluding current)
            results = [(page, sup*0.3) for page, sup in self.global_popular_pages 
                      if page != current_page and sup <= 100][:top_n]
        
        return results
    
    def get_related_pages(self, current_page, top_n=5):
        """Get pages that actually appear together in patterns"""
        related = []
        
        if current_page in self.pattern_database:
            # Get all related pages from patterns (excluding next/prev for co-occurrence)
            for typ, page, support in self.pattern_database[current_page]:
                if typ == 'related' and page != current_page and support <= 100:
        ...
        # If no pattern-based related pages, use contextual
        if not results:
            contextual_recs = self.get_contextual_recommendations(current_page)
            results = [(page, sup*0.6) for page, sup in contextual_recs]
        
        return results[:top_n]
    
    def get_contextual_recommendations(self, current_page, top_n=5):
        """Get contextual recommendations based on page type"""
        contextual_map = {
            # Contest Submission related pages
            'contestsubmission': ['showcode', 'standings', 'contestproblem', 'profile', 'allsubmission'],
            'allsubmission': ['contestsubmission', 'showcode', 'standings', 'contestproblem'],
            ...
            'profile': ['contestsubmission', 'standings', 'archive', 'allsubmission'],
            
            # Navigation and browsing
            '1': ['contestproblem', 'archive', 'standings', 'login'],
            'archive': ['contestproblem', 'details', 'description', 'standings'],
            'standings': ['contestsubmission', 'contestproblem', 'allsubmission'],
            
            # Code viewing
            'showcode': ['contestsubmission', 'allsubmission', 'standings'],
            
            # Contest utilities
            'countdown': ['contestproblem', 'standings', 'contestsubmission'],
            
            # Font library (usually appears with many pages)
            'fontawesome-webfont': ['1', 'contestproblem', 'profile', 'login']
        }
        ...
        if current_page in contextual_map:
            ...
            for page in contextual_pages:
                support = self._get_support_for_page_combination(current_page, page)
                enhanced_recs.append((page, support))
            return sorted(enhanced_recs, key=lambda x: x[1], reverse=True)[:top_n]
        
        return []
    
    def _get_support_for_page_combination(self, page1, page2):
        """Get support for pages appearing together"""
        max_support = 0
        for itemset, support in self.frequent_itemsets:
            if page1 in itemset and page2 in itemset:
                support_pct = (support / self.total_transactions) * 100
                if support_pct > max_support and support_pct <= 100:
                    max_support = support_pct
        return max_support if max_support > 0 else 5.0  # Default low confidence
    ...
    def get_popular_flows(self, min_support=2.0):
        """Get popular user navigation flows with CORRECT support"""
        popular_flows = []
        for itemset, support in self.frequent_itemsets:
            if len(itemset) > 1:
                support_pct = (support / self.total_transactions) * 100
                if support_pct >= min_support and support_pct <= 100:
    ...
    def describe_page(self, page):
        ...
        return self.page_descriptions.get(page, f"{page} (System Page)")
...
def analyze_frequent_patterns(transactions, min_support=0.01):
    """Complete FP-tree analysis pipeline with CORRECT support"""
    ...
    # Filter out itemsets with impossible support values
    valid_itemsets = []
    for itemset, support in frequent_itemsets:
        ...
        if support_pct <= 100:  # Only include valid percentages
            ...
            print(f"DEBUG: Filtered out itemset with invalid support: {itemset} - {support_pct:.1f}%")
    
    print(f"Valid frequent itemsets: {len(valid_itemsets)}")
    
    return fp_tree, valid_itemsets
...
def display_recommendations(recommendation_system, current_page):
    """Display FIXED recommendations for the current page"""
    ...
    if next_recs:
        ...
        for i, (page, support) in enumerate(next_recs, 1):
            description = recommendation_system.describe_page(page)
            # Ensure support is reasonable
            display_support = min(support, 100.0)
            print(f"  {i}. {description:<25} - {display_support:.1f}% confidence")
    else:
        print("\n📈 No specific next page patterns found.")
    ...
    if related_recs:
        ...
        for i, (page, support) in enumerate(related_recs, 1):
            description = recommendation_system.describe_page(page)
            display_support = min(support, 100.0)
            print(f"  {i}. {description:<25} - {display_support:.1f}% co-occurrence")
    
    # Show contextual suggestions
    contextual_recs = recommendation_system.get_contextual_recommendations(current_page)
    if contextual_recs and (not next_recs or len(next_recs) < 3):
        print("\n💡 CONTEXTUAL SUGGESTIONS:")
        for i, (page, support) in enumerate(contextual_recs[:3], 1):
            description = recommendation_system.describe_page(page)
...
def display_popular_flows(recommendation_system):
    ...
    popular_flows = recommendation_system.get_popular_flows(min_support=2.0)
